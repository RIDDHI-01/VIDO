...
export default function Video(){
    ...
    let silence=()=>{
        let ctx=new AudioContext() //creates an audio context for audio processing
        let oscillator=ctx.createOscillator(); //basically generates a constant tone
    ...
    let black=({width=640,height=480}={})=>{
        ...
        canvas.getContext('2d').fillRect(0,0,width,height); //creating black rectangle
        let stream=canvas.captureStream(); //capturing black screen
    ...
    let connectToSocketServer=()=>{
        ...
        socketRef.current.on("user-joined",(id,clients)=>{
            clients.forEach((socketListId)=>{
                ...
                //ice is a protocol-> interactive connectivity establishment, building a direct
                ...
                if(window.localStream!=undefined && window.localStream!=null){
                    ...
                    //black screen when video is off, by using canvas
                    let blackSilence=(...args)=>new MediaStream([black(...args),silence()])
                    window.localStream=blackSilence();
                    connections[socketListId].addStream(window.localStream);
                }
            ...
            if(id===socketIdRef.current){
                for(let id2 in connections){
                    if(id2===socketIdRef.current) continue; //if it is me only
                    try{
            ...
            <div className={styles.meetVideoContainer}>
                {showModel?<div className={styles.chatRoom}>
                    ...
                    <div className={styles.chattingArea}>
                        <TextField value={message} onChange={(e)=>setMessage(e.target.value)} id='outlined-basic' label='Enter message'></TextField>
                ...
                <div className={styles.btnContainer}>
                    ...
                    <Badge badgeContent={newMessages} max={999} color='secondary'>
                        <IconButton onClick={()=>{setModel(!showModel)}} style={{color:"white"}}><ChatIcon/></IconButton>
                    </Badge>
                </div>
